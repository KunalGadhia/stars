...
//                        $scope.years.reverse();
                                ...
                                if ($scope.placeHolders)
                                    $scope.years.unshift($scope.placeHolders[0]);
                            }

                            // Verify if selected date is in the valid range.
                            if ($scope.ngModel && $scope.ngMinModel && $scope.ngModel < $scope.ngMinModel)
                                $scope.ngModel = $scope.ngMinModel;
                            if ($scope.ngModel && $scope.ngMaxModel && $scope.ngModel > $scope.ngMaxModel)
                        ...
                        $scope.updateMonthList = function (year) {
                            ...
                            $scope.months = [];
                            if ($scope.placeHolders)
                                $scope.months.push($scope.placeHolders[1]);
                        ...
                        $scope.updateDateList = function (month, year) {
                            ...
                            $scope.dates = [];
                            if ($scope.placeHolders)
                                $scope.dates.push($scope.placeHolders[2]);
                            for (var i = start; i <= end; i++) {
                ...
                link: function (scope, element, attrs, ngModelCtrl) {
                    ...
                    for (var i = 0; i < order.length; i++) {
                        if (order[i] == 'd')
                            jqLite(element[0]).append(children[0]);
                        if (order[i] == 'm')
                            jqLite(element[0]).append(children[1]);
                        if (order[i] == 'y')
                            jqLite(element[0]).append(children[2]);
                    }
                    ...
                    ngModelCtrl.$formatters.push(function (modelValue) {
                        ...
                        if (modelValue) {
                            console.log("Model Value :%O", modelValue);
                            var date = new Date(modelValue);
                            console.log("Date :%O", date.getDate());
                            console.log("Month :%O", date.getMonth());
                            console.log("Year :%O", date.getFullYear());
...
//                                milliseconds = scope.ngModel.getMilliseconds();
                        ...
                        if (scope.placeHolders && angular.isUndefined(scope.ngPlaceholderEnabled)) {
                            if (scope.year != '')
                                scope.placeHolders[0].disabled = true;
                            if (scope.month != '')
                                scope.placeHolders[1].disabled = true;
                            if (scope.date != '')
                                scope.placeHolders[2].disabled = true;
                        }
